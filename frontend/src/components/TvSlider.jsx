...
import { useGetTvsByCategoryQuery } from "../redux/api/tvApiSlice";
import { Link } from "react-router-dom";
import { SMALL_IMG_BASE_URL } from "../utils/constants";
import { useRef, useState } from "react";
import { ChevronLeft, ChevronRight } from "lucide-react";

export default function TvSlider({ category }) {
  const { type } = useSelector((state) => state.contentType);

  const [showArrows, setShowArrows] = useState(false);
  const sliderRef = useRef(null);

  const { data: getTvsByCategory } = useGetTvsByCategoryQuery(category);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        ref={sliderRef}
      >
        {getTvsByCategory?.content.map((tv) => (
          <Link
            to={`/watch/${tv.id}`}
            key={tv.id}
          ...
          >
            <div className="rounded-lg overflow-hidden">
              <img
                src={SMALL_IMG_BASE_URL + tv.backdrop_path}
                alt="tv image"
                className="transition-transform duration-300 ease-in-out group-hover:scale-125"
              />
            </div>
            <p className="mt-2 text-center">{tv.title}</p>
          </Link>
        ))}
      </div>
